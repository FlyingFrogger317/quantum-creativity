...
public class QuantizingJEIPlugin implements IModPlugin {
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        registration.addRecipes(QuantizingRecipeCategory.recipeType,Objects.requireNonNull(Minecraft.getInstance().level).getRecipeManager().getAllRecipesFor(QuantizingRegistries.QuantizingRecipeType.get()).stream().map(RecipeHolder::value).collect(java.util.stream.Collectors.toList()));
    }
}
