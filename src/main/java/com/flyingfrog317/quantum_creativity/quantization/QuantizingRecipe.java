...
public class QuantizingRecipe implements Recipe<SingleRecipeInput> {
    public final Ingredient inputs;
    public final NonNullList<Ingredient> outputs;

    public QuantizingRecipe(Ingredient input, NonNullList<Ingredient> outputs) {
        this.inputs = input;
        this.outputs = outputs;
    }
    @Override
    public boolean matches(SingleRecipeInput simpleContainer, Level level) {
    ...
    @Deprecated
    @Override
    public ItemStack assemble(SingleRecipeInput simpleContainer, HolderLookup.Provider registryAccess) {
