package com.flyingfrog317.quantum_creativity;

import net.minecraft.Util;
import net.minecraft.world.item.ArmorItem;

import java.util.EnumMap;
import java.util.Map;

public class ItemBalancing {
    public static final int SINGLE_STACK_SIZE = 1;
    public static final int SMALL_STACK_SIZE = 16;
    public static final int STANDARD_STACK_SIZE = 64;
    public static final float ATTACK_SPEED_NORMALIZER = -4f;
    public static final int QUANTUM_TOOL_DURABILITY = 10000;

    public static final int QUANTUM_AXE_DAMAGE = 100;
    public static final float QUANTUM_AXE_ATTACK_SPEED = 1.2f;

    public static final int QUANTUM_PICKAXE_DAMAGE = 30;
    public static final float QUANTUM_PICKAXE_ATTACK_SPEED = 0.8f;

    public static final int QUANTUM_SWORD_DAMAGE = 50;
    public static final float QUANTUM_SWORD_ATTACK_SPEED = 3f;

    public static final int QUANTUM_SHOVEL_DAMAGE = 30;
    public static final float QUANTUM_SHOVEL_ATTACK_SPEED = 0.8f;

    public static final int QUANTUM_HOE_DAMAGE = 5;
    public static final float QUANTUM_HOE_ATTACK_SPEED = 0.5f;

    public static final int QUANTUM_COMPONENT_STACK_SIZE = SMALL_STACK_SIZE;
    public static final float DAMAGED_QUANTUM_CASING_DESTROY_TIME = 0.2f;
    public static final float QUANTUM_CASING_DESTROY_TIME = 1f;
    public static final float QUANTIZER_ANIMATION_SPEED = 0.2f;

    public static final int LIQUID_QUANTUM_METAL_DENSITY = 3000;
    public static final int LIQUID_QUANTUM_METAL_VISCOSITY = 6000;
    public static final int LIQUID_QUANTUM_METAL_TEMPERATURE = 1300;

    public static final float SCHRODINGER_BOX_STRENGTH = 0.5f;

    public static final float QUANTIZER_DESTROY_TIME = 1.5f;
    public static final float QUANTIZER_EXPLOSION_RESISTANCE = 20f;

    public static final Map<ArmorItem.Type, Integer> QUANTUM_ARMOR_DEFENCE_MAP = Util.make(new EnumMap<>(ArmorItem.Type.class), (map) -> {
        map.put(ArmorItem.Type.HELMET, 17);
        map.put(ArmorItem.Type.CHESTPLATE, 23);
        map.put(ArmorItem.Type.LEGGINGS, 20);
        map.put(ArmorItem.Type.BOOTS, 15);
    });
    public static final int QUANTUM_ARMOR_ENCHANTABILITY = 60;
    public static final float QUANTUM_ARMOR_TOUGHNESS = 10;
    public static final float QUANTUM_ARMOR_KNOCKBACK_RESIST = 10;
}
